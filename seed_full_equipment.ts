...
async function main() {
  ...
  await prisma.equipmentVote.deleteMany({});
  await prisma.equipmentPriorityVote.deleteMany({});
  ...
  console.log('Seeding full Equipment Data...');
  ...
  // Categories
  const catMaschinen = await prisma.equipmentCategory.create({ data: { title: '1. Maschinen' } });
  const catSiebdruck = await prisma.equipmentCategory.create({ data: { title: '2. Siebdruck-Set' } });
  const catHolz = await prisma.equipmentCategory.create({ data: { title: '3. Einrichtung Holzwerkstatt' } });
  const catMoebel = await prisma.equipmentCategory.create({ data: { title: '4. Möbel' } });
  ...
  console.log('Seeding Complete! Total budget set to 60886.46');
}
